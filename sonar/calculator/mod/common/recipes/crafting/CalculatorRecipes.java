...
public class CalculatorRecipes {
	...
	public boolean discovery(int[] unblocked, int latest) {
		/*
		 * if (stack.getItem() == Calculator.itemCalculator) {
		 * 
		 * int[] unblocked = CalculatorItem.getResearch(stack); int[] newRecipes = player.newRecipes(entity); Map<Integer, CalculatorRecipe> recipes = CalculatorRecipes.recipes().getStandardList();
		 * for (Map.Entry<Integer, CalculatorRecipe> recipe : recipes.entrySet()) { if (newRecipes != null && newRecipes.length >= 1) { if (recipe.getValue().hidden &&
		 * newRecipes[CalculatorRecipes.recipes().getID(recipe.getValue().input)] != 0 || newRecipes[CalculatorRecipes.recipes().getID(recipe.getValue().input2)] != 0 ||
		 * newRecipes[CalculatorRecipes.recipes().getID(recipe.getValue().output)] != 0) { if (unblocked[CalculatorRecipes.recipes().getID(recipe.getValue().input)] != 0 &&
		 * unblocked[CalculatorRecipes.recipes().getID(recipe.getValue().input2)] != 0 || unblocked[CalculatorRecipes.recipes().getID(recipe.getValue().output)] != 0) { return true; } } } } }
		 */
