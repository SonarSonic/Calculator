...
public class InvisibleBlock extends Block implements IWrench {
			...
			if (!world.isRemote) {
				...
				if (world.getBlock(x, y - 1, z) == Calculator.conductorMast) {
					...
					player.openGui(Calculator.instance,
							CalculatorGui.ConductorMast, world, x, y - 1, z);
				} else if (world.getBlock(x, y - 2, z) == Calculator.conductorMast) {

					player.openGui(Calculator.instance,
							CalculatorGui.ConductorMast, world, x, y - 2, z);
				} else if (world.getBlock(x, y - 3, z) == Calculator.conductorMast) {

					player.openGui(Calculator.instance,
							CalculatorGui.ConductorMast, world, x, y - 3, z);
				}
				}
			}
		}
	...
	public final boolean removedByPlayer(World world, EntityPlayer player,
		...
		if(this.type==0){
			if (world.getBlock(x, y - 1, z) == Calculator.conductorMast) {	
				...
				SonarHelper.dropTile(world.getTileEntity(x, y - 1, z), player, world.getBlock(x, y - 1, z), world, x, y-1, z);
			} else if (world.getBlock(x, y - 2, z) == Calculator.conductorMast) {
				TileEntityConductorMast mast = (TileEntityConductorMast) world.getTileEntity(x, y - 2, z);
				SonarHelper.dropTile(world.getTileEntity(x, y - 2, z), player, world.getBlock(x, y - 2, z), world, x, y-2, z);
			} else if (world.getBlock(x, y - 3, z) == Calculator.conductorMast) {
				TileEntityConductorMast mast = (TileEntityConductorMast) world.getTileEntity(x, y - 3, z);
				SonarHelper.dropTile(world.getTileEntity(x, y - 3, z), player, world.getBlock(x, y - 3, z), world, x, y-3, z);
			}
			}
		else if(this.type==1){
				for(int X =-1; X<2; X++){
