...
public class AtomicMultiplier extends SonarMachineBlock implements IWrench {
		...
		if (player != null) {
			if (!world.isRemote) {
				player.openGui(Calculator.instance,
						CalculatorGui.AtomicMultiplier, world, x, y, z);
			}
		}
		return true;
	}
	...
	public void randomDisplayTick(World world, int x, int y, int z,
			Random random) {
		TileEntityAtomicMultiplier te = (TileEntityAtomicMultiplier) world
				.getTileEntity(x, y, z);
		if (te.active==1) {
	...
	public void standardInfo(ItemStack stack, EntityPlayer player, List list) {
		if (CalculatorConfig.energyStorageType == 2) {
			list.add(StatCollector.translateToLocal("energy.required") + ": "
					+ TileEntityAtomicMultiplier.requiredEnergy / 4
					+ " EU");

		} else {
			list.add(StatCollector.translateToLocal("energy.required") + ": "
					+ TileEntityAtomicMultiplier.requiredEnergy + " RF");
		}
	}
}
