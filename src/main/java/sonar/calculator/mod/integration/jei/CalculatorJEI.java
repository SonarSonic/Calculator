...
public class CalculatorJEI extends BlankModPlugin {
	...
	public enum Handlers implements IJEIHandler {
		...
		Handlers(RecipeHelper helper, String unlocalizedName, String textureName, Class<? extends JEIRecipe> recipeClass) {
			this.helper = helper;
		...
		public JEICategory getCategory(IGuiHelper guiHelper){
			switch(this){
			case PROCESSING: case RESTORATION: case REASSEMBLY:
				return new SingleProcessCategory(guiHelper, this);
			case EXTRACTION: case PRECISION: case STONE: case ALGORITHM:
				return new DualProcessCategory(guiHelper, this);				
			case CALCULATOR: case SCIENTIFIC:
