...
public class CalculatorHandler {
	...
	public static void addRecipe(IIngredient input1, IIngredient input2, IItemStack output) {
		MineTweakerAPI.apply(new SonarAddRecipe(RecipeRegistry.CalculatorRecipes.instance(), Lists.newArrayList(input1, input2), Lists.newArrayList(MineTweakerMC.getItemStack(output))));
	}

	@ZenMethod
	public static void removeRecipe(IIngredient input1, IIngredient input2) {
		MineTweakerAPI.apply(new SonarRemoveRecipe(RecipeRegistry.CalculatorRecipes.instance(), Lists.newArrayList(input1, input2), true));
	}
}
