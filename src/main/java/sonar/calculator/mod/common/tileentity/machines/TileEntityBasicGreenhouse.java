...
public class TileEntityBasicGreenhouse extends TileEntityBuildingGreenhouse implements ISidedInventory, IGuiTile {
	...
	public boolean hasRequiredStacks() {
		if (slots().get(0) != null && slots().get(1) != null && slots().get(2) != null && slots().get(3) != null) {
			if (SonarCompat.getCount(slots().get(0)) >= requiredLogs && GreenhouseHelper.checkLog(Block.getBlockFromItem(slots().get(0).getItem()))) {
				if (SonarCompat.getCount(slots().get(1)) >= requiredStairs && GreenhouseHelper.checkStairs(Block.getBlockFromItem(slots().get(1).getItem()))) {
					if (SonarCompat.getCount(slots().get(2)) >= requiredGlass && GreenhouseHelper.checkGlass(Block.getBlockFromItem(slots().get(2).getItem()))) {
						if (SonarCompat.getCount(slots().get(3)) >= requiredPlanks && GreenhouseHelper.checkPlanks(Block.getBlockFromItem(slots().get(3).getItem()))) {
							return true;
						}
					}
				}
