			...
			if (coords.getDimension() != currentDimension) {
				if (!tile.getWorld().isRemote && !entity.isDead) {
					...
					if (currentDimension == 1) {
						((EntityPlayerMP) entity).connection.setPlayerLocation(coords.getX() + 0.5, coords.getY() - 2, coords.getZ() + 0.5, SonarHelper.getAngleFromMeta(worldServer.getBlockState(coords.getBlockPos()).getValue(SonarProperties.FACING).getIndex()), 0);
				...
				((EntityPlayerMP) entity).connection.setPlayerLocation(coords.getX() + 0.5, coords.getY() - 2, coords.getZ() + 0.5, SonarHelper.getAngleFromMeta(entity.world.getBlockState(coords.getBlockPos()).getValue(SonarProperties.FACING).getIndex()), 0);
			}
			tile.coolDown = true;
